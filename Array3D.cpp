...
template <typename T>
Array3D<T>::Array3D(int page, int col, int row, T value)
{
...
template <typename T>
Array3D<T>::Array3D(const vector<Mat> &vector_img)
{
...
void Array3D<T>::expand_to_full_size(int col_size, int row_size)
{
	...
	// ���������ĳߴ���չ��full�ߴ磬��ά����X.size()��ΪX.size() + 2(Ker.size() - 1)��
...
template <typename T>
void Array3D<T>::show_specified_images_64FC1(const std::string& MultiShow_WinName, Size SubPlot, Size ImgMax_Size, int time_msec) const
{
	if (_array3D.size() <= 0)
	{
...
template Array3D<double>::Array3D(int page, int col, int row, double value);
template Array3D<double>::Array3D(const vector<Mat> &vector_img);
...
template void Array3D<double>::show_specified_images_64FC1(const std::string& MultiShow_WinName, Size SubPlot, Size ImgMax_Size, int time_msec) const;
