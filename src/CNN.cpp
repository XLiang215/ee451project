...
void CNN::train(const Array3Dd &train_x, const Array2Dd &train_y)
{
    ...
    cout<<"total training sample: "<<m<<endl;
    ...
    for (int I = 0; I < _epochs; I++)// 比如整体重复迭代25次
    {
        ...
        {
            if (L % 1000 == 999)
                cout<<"batch "<<L + 1<<" / "<<numbatches<<endl;
...
//            cout << "                          batches " << L+1 << " feedforward time: " << (double)(toc_ff - tic_ff) / CLOCKS_PER_SEC << " seconds" << endl;

            // 得到上面的网络输出后，通过对应的样本标签用bp算法来得到误差对网络权值(反向传播) 的导数
//            clock_t tic_bp = clock();
            back_propagation(batch_train_y);
//            clock_t toc_bp = clock();
//            cout << "                          batches " << L + 1 << " back propagation time: " << (double)(toc_bp - tic_bp) / CLOCKS_PER_SEC << " seconds" << endl;

            // 得到误差对权值的导数后，就通过权值更新方法去更新权值
//            clock_t tic_update = clock();
            update();
//            clock_t toc_update = clock();
//            cout << "                          batches " << L + 1 << " grade update time: " << (double)(toc_update - tic_update) / CLOCKS_PER_SEC << " seconds" << endl;
        ...
        cout << "epochs " << I+1 << " time has elapsed: " << (double)(toc - tic) / CLOCKS_PER_SEC << " seconds" << endl;
    }
    //*/
